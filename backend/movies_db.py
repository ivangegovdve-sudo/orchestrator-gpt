...
def _movie_rows_to_dicts(conn: sqlite3.Connection, rows: List[sqlite3.Row]) -> List[Dict[str, Any]]:
    ...
    # ⚡ Bolt optimization: Check for `tags_str` key presence outside the hot loop to avoid O(N) dict key lookups.
    has_tags = "tags_str" in rows[0].keys()

    for row in rows:
        tags_str = row["tags_str"] if has_tags else None
