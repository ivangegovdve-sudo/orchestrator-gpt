...
**Learning:** Found an unprotected destructive delete action in `web/shared-calendar/index.html`. Users could easily click "Delete" or "Remove" to delete data without any confirmation or way to undo.
**Action:** Add `window.confirm("Are you sure you want to remove this activity?")` checks. In the future, look for buttons labeled `Delete` or `Remove` and ensure they have confirmation dialogs attached before carrying out destructive operations.
