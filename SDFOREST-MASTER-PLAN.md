...
- **Why open:** Current old groups/trails are not target authority. Open mappings include Explore Repos, Councils, Avatar Playground, Voice Playground, Rubik’s Teacher/Cubeflow, Calendar, Kids Corner, Kids Movie Library, Poetry Space, Open Dashboard, the icon generator, FlowForm, Multiply Magic Studio, and the public documentation for Chloé PWA and Chloé desktop. Chloé’s non-public runtime boundary does not decide its public documentation’s pool placement. Verify Gym Scholar versus the current Hyper Trophy OS route; settled Gym Scholar ordering does not prove they are identical. No copied routes establish the legacy-plan-only product proposals.
