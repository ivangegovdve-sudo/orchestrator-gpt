import { ENDPOINTS, GITHUB_CATEGORIES, OVERVIEW_REQUESTS, createOpenOverviewClient, topGitHubEnrichmentRequests } from "./open-overview-api.js";
...
function renderStarBucketDisclosure(document,fullName,buckets){const visible=(Array.isArray(buckets)?buckets:[]).slice(-7);if(!visible.length){const span=document.createElement("span");span.textContent="—";span.setAttribute("aria-label","No published star buckets");return span}const values=visible.map((bucket)=>BigInt(bucket.count));const maximum=values.reduce((largest,value)=>value>largest?value:largest,0n);const details=document.createElement("details");details.className="oo-github-enrichment-disclosure";const summary=document.createElement("summary");summary.setAttribute("aria-label",`Show exact star buckets for ${fullName}`);const glyph=document.createElement("span");glyph.className="oo-github-star-sparkline";glyph.setAttribute("role","img");glyph.setAttribute("aria-label",`${visible.length} exact daily star buckets for ${fullName}`);glyph.textContent=visible.map((_bucket,index)=>starGlyph(values[index],maximum)).join("");summary.appendChild(glyph);const list=document.createElement("ol");list.setAttribute("aria-label",`Exact star bucket values for ${fullName}`);for(const bucket of visible){const item=document.createElement("li");item.textContent=`${bucket.start}: ${bucket.count}`;list.appendChild(item)}details.append(summary,list);return details}
