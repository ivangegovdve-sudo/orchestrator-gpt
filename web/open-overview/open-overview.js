import { ENDPOINTS, GITHUB_CATEGORIES, OVERVIEW_REQUESTS, createOpenOverviewClient } from "./open-overview-api.js";
...
export const OPENROUTER_VIEWS = Object.freeze({
  ...
  free: { label: "Free", requests: [{ key: "free", path: ENDPOINTS.freeModels, kind: "free", sourceId: "models_current" }, { key: "freeFrontier", path: ENDPOINTS.freeFrontier, kind: "freeFrontiers", optional: true }] },
...
const OVERVIEW_DEFERRED_KEYS = new Set(["providers", "freeFrontier", "history"]);
...
export function parseOpenRouterState(url) {
  ...
  return Object.freeze({ view: requested in OPENROUTER_VIEWS ? requested : "usage", appId: appId && /^\d{1,40}$/.test(appId) ? appId : null, freeMode: query.get("freeMode") === "pareto" ? "pareto" : "popularity" });
}
...
const OPTIONAL_DATASET_KEYS = new Set(["matrix", "providers", "freeFrontier", "history", "appModels"]);
const isOptionalDataset = (key) => OPTIONAL_DATASET_KEYS.has(key) || key.startsWith("github:") || key.startsWith("momentum:") || key.startsWith("fallback:");
...
export function buildSourceRows(view) {
  const rows = view.manifest.sources.map((source) => {
    ...
    return { datasetKey: `source:${source.sourceId}`, sourceId: source.sourceId, required: true, mode: view.mode, freshness: source.stale || response?.stale || response?.coverage?.stale || view.snapshotStale ? "stale" : "current", completeness: source.publishedRunId === null ? "unavailable" : response?.completeness ? population(response.completeness.acquisitionComplete, response.completeness.populationCompleteness) : response?.coverage ? population(response.coverage.acquisitionComplete, response.coverage.populationCompleteness) : population(source.lastAttemptAcquisitionComplete ?? true, source.lastAttemptPopulationCompleteness ?? "partial_or_unknown"), asOf: response?.provenance?.[0]?.sourceAsOf ?? source.publishedAt };
  });
  for (const [key, response] of Object.entries(view.responses)) {
    if (key.startsWith("github:") || key === "ranking" || key.startsWith("momentum:")) rows.push({ datasetKey: key, sourceId: `github.${response.ranking.metric}:${response.ranking.category}`, required: !isOptionalDataset(key), mode: view.mode, freshness: response.coverage.stale || view.snapshotStale ? "stale" : "current", completeness: population(response.coverage.acquisitionComplete, response.coverage.populationCompleteness), asOf: response.coverage.resolvedAsOf });
...
export function renderHistoryPanel(view, seriesKey, title) {
  ...
  if (buckets.length < 2) return renderUnavailable({ document, title, reason: "insufficient_history" });
  ...
  for (const target of buckets.at(-1).rows.slice(0, 10)) { const values = buckets.map((bucket) => bucket.rows.find((row) => row.id === target.id)).filter(Boolean).map((row) => row.value ?? String(row.rank)); if (values.length !== buckets.length) continue; const line = document.createElement("div"); line.className = "oo-history-line"; const label = document.createElement("span"); label.textContent = target.label; line.append(label, renderSparkline({ document, values, label: `${target.label} exact series: ${values.join(", ")}` })); region.appendChild(line); }
  const details = document.createElement("details"); const summary = document.createElement("summary"); summary.textContent = "Exact history values"; details.append(summary, renderRankTable({ document, title: `${title} exact values`, rows: buckets.flatMap((bucket) => bucket.rows.map((row) => ({ ...row, date: bucket.date }))), sourceLabel: "Approved-run history", asOf: buckets.at(-1).date, columns: [{ label: "Date", value: (row) => row.date }, { label: "Item", value: (row) => row.label }, { label: "Scope", value: (row) => exact(row.scope) }, { label: "Rank", value: (row) => exact(row.rank) }, { label: "Value", value: (row) => exact(row.value) }, { label: "Remainder", value: (row) => exact(row.remainder) }, { label: "Stars", value: (row) => exact(row.stars) }, { label: "Forks", value: (row) => exact(row.forks) }] })); region.appendChild(details); return region;
}
...
const mergeCompatibleViews = (primary, deferred) => {
...
function installOverviewDeferredLoad(client, initialView) {
  ...
  const load = async () => { target.dataset.deferredState = "loading"; try { const deferred = await client.loadView(OVERVIEW_DEFERRED_REQUESTS); hydrateOverviewDeferred(mergeCompatibleViews(initialView, deferred)); target.dataset.deferredState = "ready"; } catch (error) { const failed = Object.fromEntries(OVERVIEW_DEFERRED_REQUESTS.map((spec) => [spec.key, error])); hydrateOverviewDeferred(Object.freeze({ ...initialView, errors: Object.freeze({ ...initialView.errors, ...failed }) })); target.dataset.deferredState = "failed"; } };
...
export function renderOpenRouter(view, state) {
  ...
  else if (state.view === "free" && state.freeMode === "pareto") { content.appendChild(freeModeNav(document,state)); const frontier=envelopeRows(view,"freeFrontier")[0]; content.appendChild(frontier ? renderRankTable({ document,title:"Free Pareto frontier",rows:frontier.members,sourceLabel:`${frontier.ruleVersion} · ${frontier.dimensions.x} ${frontier.dimensions.xDirection} × ${frontier.dimensions.y} ${frontier.dimensions.yDirection}`,asOf:view.responses.freeFrontier?.window?.end,columns:[{label:"Model",value:(row)=>row.modelId},{label:frontier.dimensions.x,value:(row)=>row.x},{label:frontier.dimensions.y,value:(row)=>row.y}] }) : renderUnavailable({document,title:"Free Pareto frontier",reason:`Frontier enrichment is unavailable (${view.errors.freeFrontier?.details?.apiCode || "unavailable"}). Popularity remains available; no composite efficiency score is substituted.`})); }
...
export function renderGithub(view,state){const{document,root}=context();const envelope=view.responses.ranking;root.replaceChildren(githubControls(document,state,view));renderSourceRail(view);appendSnapshotNotice(document,root,view);const content=section(document,"oo-github-content","oo-route-content");const rows=Array.isArray(envelope?.data)?envelope.data:[];const definition=GITHUB_METRICS.find(([slug])=>slug===state.metric)[1];content.appendChild(renderRankTable({document,title:`${definition} · ${GITHUB_CATEGORIES.find(([slug])=>slug===state.category)[1]}`,rows,sourceLabel:"GitHub project-family ranking",asOf:envelope?.coverage.resolvedAsOf,columns:[{label:"Rank",value:(row)=>row.rank},{label:"Project",value:(row)=>row.fullName,href:(row)=>`https://github.com/${row.fullName}`},{label:"Stars",value:(row)=>compactIntegerString(row.stars),exact:(row)=>row.stars},{label:"Forks",value:(row)=>compactIntegerString(row.forks),exact:(row)=>row.forks},{label:definition,value:(row)=>exact(row.score)}]}));const meta=document.createElement("p");meta.className="oo-ranking-meta";meta.textContent=envelope?.ranking?`${envelope.ranking.rankMethod} · ${envelope.ranking.ruleVersion} · Taxonomy: ${envelope.ranking.taxonomyVersion} · Eligible population: ${envelope.ranking.eligiblePopulation}`:"Ranking metadata unavailable";const method=document.createElement("p");method.className="oo-ranking-method";method.textContent=state.metric==="adoption"?"Adoption = 0.75 × percent_rank(log1p(stars)) + 0.25 × percent_rank(log1p(forks)); raw stars and forks remain visible.":state.metric==="momentum"?"Momentum requires a fully covered 7, 30, or 90-day window; incomplete windows are ineligible.":"Maintenance is a recency ranking using default-branch commit and stable-release evidence, not a health score.";content.append(meta,method,renderHistoryPanel(view,"githubRanks","GitHub rank over time"));root.appendChild(content);root.setAttribute("aria-busy","false")}
