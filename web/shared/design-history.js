...
(() => {
  ...
  /* The landing page (forest-home.css) declares its own `--home-*` set
     with literal colours rather than deriving them from `--forest-*`.
     Rather than edit that stylesheet — this feature must not touch the
     current design — we mirror each era's colours onto the `--home-*`
     names at runtime, so the landing re-tints along with everything else.
     Fonts already alias through, so only colours are listed here. */
  const HOME_ALIASES = {
    "--home-bg": "--forest-bg",
    "--home-ink": "--forest-ink",
  ...
  function withAliases(tokens) {
    ...
    Object.entries(HOME_ALIASES).forEach(([alias, source]) => {
  ...
  drawer.innerHTML =
    '<div class="dh-head">' +
      "<div>" +
        ...
        '<p class="dh-sub">Five eras · since Dec 2025</p>' +
      "</div>" +
