...
The build is wired to the installed `open-dashboard-mcp` package: a real in-memory MCP handshake verifies its 1.1.4 tool contract, the package-backed collectors refresh the dated native snapshot, and the explorer combines those records with paginated public rows plus the authenticated Nous read-only snapshot. Every row keeps its exact `provider:id` key, so the provider comparison can show overlap and prices without inventing aliases.
