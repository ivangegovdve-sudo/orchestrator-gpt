...
const validateAliasRegistryDrift = (raw, name) => {
  if (raw === null || raw === undefined) return null;
  const row = strictRecord(raw, ["status", "checkedAt", "rankingAsOf", "registryPublishedAt", "uncovered", "dropped", "uncoveredCount", "droppedCount", "errorCode"], name);
  if (!["clear", "registry_stale", "collection_not_run", "collection_running", "collection_failed", "check_failed"].includes(row.status)) fail("invalid_manifest", `${name}.status is invalid`);
  dateTime(row.checkedAt, `${name}.checkedAt`); if (row.rankingAsOf !== null) dateTime(row.rankingAsOf, `${name}.rankingAsOf`); if (row.registryPublishedAt !== null) dateTime(row.registryPublishedAt, `${name}.registryPublishedAt`);
...
export function validateManifest(raw, expectedMajor = "2") {
  ...
  const sources = row.sources.map((item, index) => { const source = openRecord({ ...item, lastSuccessAt: item.lastSuccessAt ?? null, consecutiveFailureCount: item.consecutiveFailureCount ?? "0", failureEscalationThreshold: item.failureEscalationThreshold ?? 3, failureEscalated: item.failureEscalated ?? false }, SOURCE_KEYS, `manifest.sources[${index}]`); string(source.sourceId, "sourceId"); if (!SOURCE_TIERS.has(source.sourceTier)) fail("invalid_manifest", `manifest source tier is invalid: ${source.sourceId} declares ${JSON.stringify(source.sourceTier)}`); if (typeof source.stale !== "boolean") fail("invalid_manifest", `manifest source state is invalid: ${source.sourceId} declares stale ${JSON.stringify(source.stale)}`); if (!Number.isInteger(source.cadenceSeconds) || source.cadenceSeconds < 1 || !Number.isInteger(source.staleAfterSeconds) || source.staleAfterSeconds < 1) fail("invalid_manifest", "manifest source cadence is invalid"); if (source.publishedRunId !== null) uuid(source.publishedRunId, "publishedRunId"); if (source.publishedAt !== null) dateTime(source.publishedAt, "publishedAt"); if (source.nextScheduledAt !== null) dateTime(source.nextScheduledAt, "nextScheduledAt"); string(source.transformVersion, "transformVersion"); if (source.lastAttemptRunId !== null) uuid(source.lastAttemptRunId, "lastAttemptRunId"); if (source.lastAttemptStatus !== null && !["running", "published", "failed"].includes(source.lastAttemptStatus)) fail("invalid_manifest", "lastAttemptStatus is invalid"); if (source.lastAttemptStartedAt !== null) dateTime(source.lastAttemptStartedAt, "lastAttemptStartedAt"); if (source.lastAttemptFinishedAt !== null) dateTime(source.lastAttemptFinishedAt, "lastAttemptFinishedAt"); if (source.lastAttemptErrorCode !== null) string(source.lastAttemptErrorCode, "lastAttemptErrorCode"); if (source.lastAttemptAcquisitionComplete !== null) boolean(source.lastAttemptAcquisitionComplete, "lastAttemptAcquisitionComplete"); if (source.lastAttemptPopulationCompleteness !== null && !POPULATION.has(source.lastAttemptPopulationCompleteness)) fail("invalid_manifest", "lastAttemptPopulationCompleteness is invalid"); if (source.citationUrl !== null && !safePublicUrl(source.citationUrl)) fail("invalid_manifest", "citationUrl is not public"); if (source.lastSuccessAt !== null) dateTime(source.lastSuccessAt, "lastSuccessAt"); integerString(source.consecutiveFailureCount, "consecutiveFailureCount"); if (!Number.isInteger(source.failureEscalationThreshold) || source.failureEscalationThreshold < 1) fail("invalid_manifest", "failureEscalationThreshold is invalid"); boolean(source.failureEscalated, "failureEscalated"); if (source.failureEscalated !== (BigInt(source.consecutiveFailureCount) >= BigInt(source.failureEscalationThreshold))) fail("invalid_manifest", "failureEscalated does not match the configured threshold"); return Object.freeze({ ...source }); });
  sources.splice(0, sources.length, ...sources.map((source, index) => Object.freeze({ ...source, aliasRegistryDrift: validateAliasRegistryDrift(row.sources[index].aliasRegistryDrift ?? null, `manifest.sources[${index}].aliasRegistryDrift`) })));
