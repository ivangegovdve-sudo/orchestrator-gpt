...
function renderCapabilityState() {
  ...
  $("chart").innerHTML = `<div class="capability-state-view"><div class="state-provider-summary" aria-label="Provider coverage">${providerSummary}</div><div class="state-query-grid">${queryCard("PUBLIC · literal cheapest paid", publicQuery)}${queryCard("PRIVATE · cheapest functional", privateQuery)}</div><div class="state-ledger-heading"><div><p class="eyebrow">Shared ledger</p><h4>Evidence per live slug</h4></div><a class="text-link" href="./capability-state.json" target="_blank" rel="noopener">Open full JSON export ↗</a></div><div class="state-table-wrap"><table class="state-table"><caption class="sr-only">Capability state rows</caption><thead><tr><th scope="col">Model</th><th scope="col">Reachability</th><th scope="col">Measured cost</th><th scope="col">Tool calling</th><th scope="col">p50 latency</th><th scope="col">Context</th></tr></thead><tbody>${rowTable}</tbody></table></div><p class="state-table-note">Showing ${visibleRows.length.toLocaleString()} of ${rows.length.toLocaleString()} rows here. The JSON export contains every available live slug and every field, including UNKNOWN and EXPIRED observations.</p></div>`;
