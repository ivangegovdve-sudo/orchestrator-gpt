...
test("the registry parser requires both the requested published release and the latest dist-tag", () => {
  ...
  for (const invalid of [
    { name: "other", "dist-tags": { latest: "1.1.7" }, versions: { "1.1.6": {}, "1.1.7": {} } },
    { name: "open-dashboard-mcp", "dist-tags": { latest: "1.1.7" }, versions: { "1.1.6": {} } },
    { name: "open-dashboard-mcp", "dist-tags": { latest: "1.1.6" }, versions: { "1.1.7": {} } },
  ])
...
test("release lookup caches a valid registry result with visible age and rejects stale cache", async () => {
  ...
  assert.deepEqual(live.facts.published, ["1.1.7"]);
...
test("the release mount visibly names only the current published release", async () => {
  ...
  assert.equal(root.value.textContent, "Published on npm: 1.1.7");
