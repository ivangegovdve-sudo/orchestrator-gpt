...
test('landing keeps one title crown and exposes Life in Time in the 30-card directory', () => {
  ...
  assert.match(landingSource, /data-project="chair-ladder"[^>]+data-href="\/web\/chair-or-ladder\//);
  assert.equal((landingSource.match(/<(?:a|article)\b[^>]*\bclass="[^"]*\bportal\b[^"]*"/g) || []).length, 30);
...
test('desktop directory starts with seventeen featured cards and a complete 30-entry index', async () => {
  const page = await browser.newPage({ viewport: { width: 1280, height: 800 } });
  await page.goto(baseUrl, { waitUntil: 'domcontentloaded' });

  assert.equal(await visiblePortalCount(page), 17);
  assert.equal(await page.locator('[data-directory-index] [data-index-project]').count(), 30);
...
test('a disclosure reveals only its own overflow cards and restores the featured directory when closed', async () => {
  ...
  assert.equal(await visiblePortalCount(page), 17);
  assert.match(await summary.innerText(), /See all 7/i);
  await summary.click();
  assert.equal(await disclosure.getAttribute('open'), '');
  assert.equal(await visiblePortalCount(page), 19);
