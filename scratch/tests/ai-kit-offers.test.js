...
test('AI-d kit subpages are published from observed boundaries', () => {
  ...
  assert.match(troubleshooting, /data-source-blob="95c9936f58c5f55aa725231395065ea16f9a3522"/);
  assert.match(troubleshooting, /data-part-count="15"/);
  assert.match(troubleshooting, /data-entry-count="113"/);
  assert.equal((troubleshooting.match(/class="troubleshooting-entry"/g) || []).length, 113);
  ...
  assert.equal(
    ...
    '95c9936f58c5f55aa725231395065ea16f9a3522',
  );
  assert.equal((troubleshootingSource.match(/^## Part \d+\b/gm) || []).length, 15);
  assert.equal((troubleshootingSource.match(/^### /gm) || []).length, 113);
  for (const field of ['Symptom', 'Cause', 'Cost', 'Check']) {
