...
async function inventory() {
  return import(`${pathToFileURL(path.join(ROOT, 'web/shared/route-inventory.mjs')).href}?v=20260913a`);
}
...
test('canonical project pages retain explicit forest-back ownership links', async () => {
  ...
  for (const entry of ROUTE_INVENTORY.filter(({ state }) => state !== 'redirect')) {
    ...
    assert.match(
      source,
      /<link rel="stylesheet" href="\/web\/shared\/forest-shell\.css\?v=20260913a">/,
