...
(async()=>{
  ...
  try {
    ...
    const ids=['health','ai-d-kit','growingapp','artificial-self','tinkerbox','design-gallery','my-story'];
    for(const id of ids) {
    ...
    const vectors=await page.evaluate(()=>{
      const layers=[];
      const selectors=['[data-ecg-wave] svg','[data-ecg-flat] svg','.neural-wires','.ambient-soil-bridge svg','[data-wind-vine]','[data-wind-grass] svg'];
      for(const selector of selectors)document.querySelectorAll(selector).forEach((el,index)=>{
        ...
        copies.forEach((copy,i)=>{
          ...
          for(const property of ['fill','stroke','stroke-width','stroke-linecap','opacity'])copy.style.setProperty(property,css.getPropertyValue(property));
        });
    ...
    for(const name of ['frontpage-pool-effects.mjs','frontpage-workbench.mjs','frontpage-ambient.mjs','frontpage-resolve.mjs','frontpage-resolve.css']) {
      fs.copyFileSync(path.resolve(__dirname,'../web/shared',name),path.join(layers,name));
    }
    fs.copyFileSync(path.resolve(__dirname,'../docs/frontpage-motion.md'),path.join(layers,'README.md'));
    await context.close();
