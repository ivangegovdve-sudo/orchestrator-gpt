...
**Vulnerability:** The FastAPI application in `backend/app.py` used a highly permissive CORS configuration that included `"null"` and `"file://"` in its `allow_origins` array alongside `allow_credentials=True`.
**Learning:** Including `"null"` in `allow_origins` is extremely dangerous because any local HTML file opened in a browser (or a sandboxed iframe without `allow-same-origin`) has a "null" origin. This allows attackers to host malicious HTML files that make credentialed cross-origin requests to the API, entirely bypassing CORS protections. Allowing `"file://"` presents similar risks.
**Prevention:** Never include `"null"` or `"file://"` in `allow_origins` when `allow_credentials=True`. Only whitelist specific, required HTTP/HTTPS origins.
