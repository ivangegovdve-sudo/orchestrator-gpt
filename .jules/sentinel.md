...
## 2024-05-24 - CORS 'null' Origin Vulnerability in FastAPI
**Vulnerability:** The FastAPI application configured CORS using `CORSMiddleware` with `allow_origins` explicitly containing both `"null"` and `"file://"`, alongside `allow_credentials=True`. This allows an attacker to bypass CORS protections by embedding the application in a sandboxed iframe, which inherently sends a `"null"` origin.
**Learning:** Including `"null"` in the `allow_origins` list when `allow_credentials=True` is an extremely severe vulnerability. It essentially nullifies the same-origin policy's protections against cross-origin reads because sandboxed iframes will always send a `"null"` origin.
**Prevention:** Never include `"null"` or `"file://"` in CORS `allow_origins`, especially when credentials are allowed. Only specify explicit, trusted origin domains.
