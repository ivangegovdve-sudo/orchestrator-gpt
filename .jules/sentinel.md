...
**Vulnerability:** The CORS configuration in `backend/app.py` allowed `"null"` and `"file://"` origins alongside `allow_credentials=True`. This permitted attackers to exploit sandboxed iframes or file protocols to bypass CORS protections.
**Learning:** Including `"null"` in `allow_origins` provides no real security because attackers can easily generate a `"null"` origin using a sandboxed iframe (`<iframe sandbox="...">`). Combined with `allow_credentials=True`, this allows full unauthorized access to authenticated endpoints.
**Prevention:** Never include `"null"` or `"file://"` in CORS `allow_origins`, especially when `allow_credentials` is enabled. Only allow explicitly trusted domains or explicitly configure local development environments correctly (e.g., specific `localhost` ports).
